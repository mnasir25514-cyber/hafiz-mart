...
async function loadPageComponents() {
    const headerTarget = document.getElementById('header') || document.getElementById('header-placeholder');
    const footerTarget = document.getElementById('footer') || document.getElementById('footer-placeholder');

    if (headerTarget) {
        const headerHtml = await loadComponent('../components/header.html').catch(() => '');
        headerTarget.innerHTML = headerHtml;
    }

    if (footerTarget) {
        const footerHtml = await loadComponent('../components/footer.html').catch(() => '');
...
function renderProductCards(items = filteredProducts) {
    ...
    if (items.length === 0) {
        productList.innerHTML = '<p class="no-results">No products matched your search.</p>';
        return;
    }

    productList.innerHTML = items.map(product => {
        const sizeOptions = product.sizes
            .map(size => `<option value="${size}">${size} - ${formatCurrency(product.price[size])}</option>`)
            .join('');

        return `
            <article class="product-card" data-product-id="${product.id}">
                <h3>${product.name}</h3>
                <p class="product-price">Starting at ${formatCurrency(product.price[product.sizes[0]])}</p>
...
function updateCartSummary() {
    ...
    cartItemsEl.innerHTML = selectedItems
        .map(item => `<li>${item.name} - ${item.size} x ${item.quantity} = ${formatCurrency(item.unitPrice * item.quantity)}</li>`)
        .join('');
...
function addProductToCart(productId) {
    ...
    const existingItem = selectedItems.find(item => item.id === productId && item.size === size);
    if (existingItem) {
...
function submitOrder() {
    ...
    if (selectedItems.length === 0) {
        messageEl.textContent = 'Please add at least one product to your cart.';
        return;
    }

    if (!address || !contact) {
        messageEl.textContent = 'Delivery address and contact number are required.';
        return;
    }
...
function initReceiptPage() {
    ...
    if (!order || !Array.isArray(order.items) || order.items.length === 0) {
        receiptDetails.innerHTML = '<p class="empty-receipt">No order found. Please place an order first.</p>';
    ...
    whatsappButton.addEventListener('click', () => {
        if (typeof window.sendWhatsAppMessage === 'function') {
            window.sendWhatsAppMessage(order.items, order.total, order.address, order.contact);
            return;
        }

        const fallbackMessage = `Order Details:\n${order.items
            .map(item => `${item.name} - ${item.size} x ${item.quantity} = ${formatCurrency(item.unitPrice * item.quantity)}`)
            .join('\n')}\n\nTotal Amount: ${formatCurrency(order.total)}\nDelivery Address: ${order.address}\nContact Number: ${order.contact}`;
        window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent(fallbackMessage)}`, '_blank');
    });
}
